        ...
        <v-dialog v-model="dialog" max-width="400px" scrollable transition="dialog-bottom-transition">
          <v-card :loading="loading">
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="designation_groupe" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.designation_groupe">
                </v-text-field>
                
                <v-select label="Activé(e)" :items="[
                  { designation_groupe: 'OUI' },
                  { designation_groupe: 'NON' }
                ]" prepend-inner-icon="extension" :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                  item-text="designation_groupe" item-value="designation_groupe" v-model="svData.active"></v-select>
          ...
          <v-card-text>
            <v-simple-table>
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">designation_groupe</th>
...
export default {
  ...
  methods: {
    ...
    showModal() {
      this.dialog = true;
      this.titleComponent = "Ajout Devise ";
      this.edit = false;
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.designation_groupe;
      } else {
        this.titleComponent = "Ajout Devise ";
      }
    }
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_grande_categorie_produit?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_grande_categorie_produit`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_grande_categorie_produit/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_grande_categorie_produit/${id}`).then(
          ({ data }) => {
