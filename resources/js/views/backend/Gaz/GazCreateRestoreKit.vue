                ...
                <v-flex xs12 sm12 md6 lg6>
                    <div class="mr-1">
                      <v-select label="Selectionnez la Catégorie" prepend-inner-icon="mdi-map" dense
                        :rules="[(v) => !!v || 'Ce champ est requis']" :items="categorieList" item-text="nom_categorie_lot" item-value="id"
                        outlined v-model="svData.refCategorie" @change="fetchListDataProduit()">
                      </v-select> 
                    </div>
                </v-flex>
                <!-- <v-flex xs12 sm12 md6 lg6>
...
export default {
    ...
    methods: {
        ...
        validate() {
            ...
            try {
                ...
                this.isLoading(true);
                    ...
                    this.insertOrUpdate(
                    `${this.apiBaseURL}/insert_gaz_globale_inventaire_innitial`,
        ...
        async fetchListDataProduit() {
            try {
                const response = await this.editOrFetch(`${this.apiBaseURL}/fetch_stock_data_gaz_byserviceAndCategorie?refService=` + this.svData.refService+"&refCategorie="+this.svData.refCategorie);
                const { data } = response;
                ...
                if (data && data.data) {
                    ...
                    this.svData.detailData = donnees.map((item, index) => {
                        ...
                        return {
                            ...item,
                            refLot: item.refLot, // Utilisez item pour chaque produit
                            designation: item.nom_lot,
                            qteDisponible: item.qte,
                            qteVente: item.qte,
                            puVente: item.cmup,
                            idStockService: item.id,
                            nom_unite: item.unite_lot
                        };
                    });
                } else {
