                      ...
                      <v-card-text>
                        ...
                        <v-layout row wrap>
                          ...
                          <v-flex xs12 sm12 md12 lg12>
                            <div class="mr-1">
                              <v-autocomplete label="Selectionnez la Vente" prepend-inner-icon="home"
                                :rules="[(v) => !!v || 'Ce champ est requis']" :items="this.venteList"
                                item-text="designationVente" item-value="id" dense outlined v-model="svData.refEnteteVente"
                                chips clearable  @change="getPrice(svData.refEnteteVente)">
                              </v-autocomplete>
                            </div>
                          </v-flex>
  ...
  export default {
    data() {
      return {
        ...
        //  'id','refEntetepaie','refEnteteVente','refBanque','montant_paie',
...
// ,'active'
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.svData.libellepaie = 'Paiement Facture Vente';
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_gaz_paiement/${this.svData.id}`,
            ...
            this.svData.libellepaie = 'Paiement Facture Vente';
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_gaz_paiement2`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
          ...
          fetchListVente() {
            this.editOrFetch(`${this.apiBaseURL}/fetch_data_gaz_entete_vente_search`).then(
              ({ data }) => {
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_gaz_paiement/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_gaz_paiement/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_gaz_paiement_byentetepaie/${this.refEntetepaie}?page=`);
      },
      backPage() {
        this.$router.go(-1);
      }
