                    ...
                    <v-flex xs12 sm12 md6 lg6>
                        <div class="mr-1">
                            <v-autocomplete label="Selectionnez le Service" prepend-inner-icon="mdi-map"
                                ...
                                item-text="nom_service" item-value="id" dense outlined v-model="svData.refService" chips
                                clearable>
                            </v-autocomplete>
                        </div>
                    </v-flex>
                    ...
                    <v-flex xs12 sm12 md6 lg6>
                        <div class="mr-1">
                            <v-select label="Selectionnez la Catégorie" prepend-inner-icon="mdi-map" dense
                                ...
                                item-text="designation" item-value="id" outlined v-model="svData.refCategorie"  @change="fetch_data_produit_service_categorie()">
                            </v-select>
                        </div>
                    </v-flex>
                    <!-- <v-flex xs12 sm12 md6 lg6>
                ...
                <v-simple-table>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in svData.detailData" :key="index">
                            ...
                            <td class="short-cell">
                                <v-text-field v-model="item.qteVente" type="number" label="Qté"></v-text-field>
                            </td>
                            <td>
                                            ...
                                            <td>
                                                ...
                                                <v-menu bottom rounded offset-y transition="scale-transition">
                                                    ...
                                                    <v-list dense width="">
                                                        ...
                                                        <v-list-item link
                                                            @click="showDetailInvetaire(item.id, item.nom_service, item.refService)">
                                                            <v-list-item-icon>
...
export default {
    ...
    methods: {
        ...
        validate() {
            ...
            try {
                ...
                if (this.$refs.form.validate()) {
                    ...
                    this.insertOrUpdate(
                        `${this.apiBaseURL}/insert_vente_globale_inventaire_innitial`,
        ...
        fetch_data_produit_service_categorie()
        {
            // this.resetForm();

            this.fetchListDataProduit();
        }
        ,
        async fetchListDataProduit() {
            try {
                
                const response = await this.editOrFetch(`${this.apiBaseURL}/fetch_stock_data_byserviceAndCategorie?refService=` + this.svData.refService+"&refCategorie="+this.svData.refCategorie);
                const { data } = response;
