            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_table"></v-text-field>

                  <v-text-field label="Code" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.code_table"></v-text-field>
              ...
              <template v-slot:default>
                <thead>
                  <tr>
                    ...
                    <th class="text-left">CodeTable</th>
...
export default {
  ...
  data() {
    return {
      ...
      disabled: false,
      edit: false,
      //'id','nom_table','code_table','active'
      svData: {
        id: "",
        nom_table: "",
        code_table: "",
  ...
  methods: {
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.nom_table;
      } else {
        this.titleComponent = "Ajout Unite ";
      }
    }
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_vente_tables?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_vente_tables`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_tables/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_vente_tables/${id}`).then(
          ({ data }) => {
