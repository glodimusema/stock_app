          ...
          <v-card :loading="loading">
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                ...
                <v-text-field label="Libellé TVA" prepend-inner-icon="extension" dense
                :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.libelle_tva">
              </v-text-field>

                <v-text-field label="Montant TVA" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.montant_tva">
                </v-text-field>                 
          ...
          <v-card-text>
            <v-simple-table>
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">LibelléTVA</th>
                    <th class="text-left">MontantTVA</th>                    
...
export default {
  ...
  methods: {
    ...
    testTitle() {
      if (this.edit == true) {
        this.titleComponent = "modification de " + item.montant_tva;
      } else {
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_vente_tva?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_vente_tva`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_tva/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_vente_tva/${id}`).then(
          ({ data }) => {
