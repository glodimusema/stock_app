<template>
                  ...
                  <v-dialog v-model="dialog" max-width="400px" persistent>
                    <v-card :loading="loading">
                      <v-form ref="form" lazy-validation>
                        <v-card-title>
                          Details Production <v-spacer></v-spacer>
                        ...
                        <v-card-text>
                          ...
                          <v-text-field type="number" label="Quantité " prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.qteProduction">
                          </v-text-field>
  
                          <v-text-field type="number" label="Prix Unitaire ($) " prepend-inner-icon="event" dense
                            :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.puProduction">
                          </v-text-field>
                            ...
                            <template v-slot:default>
                              ...
                              <tbody>
                                <tr v-for="item in fetchData" :key="item.id">
                                  <td>{{ item.nom_lot }}</td>
                                  <td>{{ item.qteProduction }}</td>
                                  <td>{{ item.uniteProduction }}</td>
                                  <td>{{ item.puProduction }}$</td>
                                  <td>{{ item.PTProduction }}$</td>
                                  <td>{{ item.refEnteteProduction }}</td>
                                  <td>{{ item.taux }}</td>
                                  <td>
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.svData.uniteProduction = this.svData.nom_unite;
            this.svData.refEnteteProduction = this.refEnteteProduction;
            this.svData.author = this.userData.name;
            this.svData.refUser = this.userData.id;

            this.insertOrUpdate(
              `${this.apiBaseURL}/update_gaz_detail_production/${this.svData.id}`,
            ...
            if(this.svData.qteProduction <= this.svData.qteDisponible)
            {
                this.svData.uniteProduction = this.svData.nom_unite;
                this.svData.refEnteteProduction = this.refEnteteProduction;
                this.svData.author = this.userData.name;
                ...
                this.insertOrUpdate(
                `${this.apiBaseURL}/insert_gaz_detail_production`,
                JSON.stringify(this.svData)
              )
                .then(({ data }) => {
                  this.showMsg(data.data);
              ...
              this.editOrFetch(`${this.apiBaseURL}/fetch_single_gaz_service_stock/${idStockService}`).then(
                  ({ data }) => {
                      ...
                      donnees.map((item) => {
                        ...
                        this.svData.puProduction = item.cmup_lot;
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_gaz_detail_production/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_gaz_detail_production/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_gaz_detail_production/${this.refEnteteProduction}?page=`);
      }
