<template>
    <v-layout>
      <!--   -->
      <v-flex md12>
        <GazDetailpaiementVente ref="GazDetailpaiementVente" />
                        ...
                        <td>
                          ...
                          <v-menu bottom rounded offset-y transition="scale-transition">
                            ...
                            <v-list dense width="">
                              ...
                              <v-list-item link @click="showGazDetailpaiementVente(item.id, item.nom_module)">
                                <v-list-item-icon>
  ...
  import GazDetailpaiementVente from "./GazDetailpaiementVente.vue";
  
  export default {
    components:{
      GazDetailpaiementVente
    },
    data() {
      return {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_gaz_entete_paiement/${this.svData.id}`,
            ...
            this.svData.refUser = this.userData.id;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_gaz_entete_paiement`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_gaz_entete_paiement/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_gaz_entete_paiement/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_gaz_entete_paiement?page=`);
      },
      showGazDetailpaiementVente(refEntetepaie, name) {
  
        if (refEntetepaie != '') {
  
          this.$refs.GazDetailpaiementVente.$data.etatModal = true;
          this.$refs.GazDetailpaiementVente.$data.refEntetepaie = refEntetepaie;
          this.$refs.GazDetailpaiementVente.$data.svData.refEntetepaie = refEntetepaie;
          this.$refs.GazDetailpaiementVente.fetchDataList();
          this.$refs.GazDetailpaiementVente.get_mode_Paiement();
          this.$refs.GazDetailpaiementVente.fetchListVente();
          this.fetchDataList();
  
          this.$refs.GazDetailpaiementVente.$data.titleComponent =
