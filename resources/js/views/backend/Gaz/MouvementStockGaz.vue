<template>
  ...
  export default {
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_gaz_mouvement_stock/${this.svData.id}`,
            ...
            this.svData.refUser = this.userData.id;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_gaz_mouvement_stock`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_gaz_mouvement_stock/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_gaz_mouvement_stock/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_gaz_mouvement_stock/${this.idStockService}?page=`);
      },
      getPrice(idStockService) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_gaz_service_stock/${idStockService}`).then(
            ({ data }) => {
                ...
                donnees.map((item) => {
                  this.svData.refLot = item.refLot;
                  this.svData.qteDisponible = item.qte_lot;
                  this.svData.puMvt=item.cmup_lot;
                });
                // this.getSvData(this.svData, data.data[0]);           
            }
        );
