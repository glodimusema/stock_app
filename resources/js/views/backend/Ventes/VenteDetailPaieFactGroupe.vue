  ...
  export default {
    data() {
      return {
        ...
        etatModal: false,
        titleComponent: '',
        refEnteteVenteGroup: 0,
  
        totalFacture: 0,
        totalPaie: 0,
        RestePaie: 0,
        //'id','refEntetepaieGroup','refEnteteVenteGroup','refBanque','montant_paie',
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_detail_paiement_groupe/${this.svData.id}`,
            ...
            this.svData.libellepaie = 'Paiement Facture';
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_detail_paiement_groupe`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_detail_paiement_groupe/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_detail_paiement_groupe/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_detail_paiement_groupe_byentete/${this.refEnteteVenteGroup}?page=`);
      },
      backPage() {
        this.$router.go(-1);
      }
