  ...
  export default {
    ...
    data() {
      return {
        ...
        refEnteteVente: 0,
        refService :0,
        //'id','refEnteteVente','idStockService','puVente','qteVente',
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.insertOrUpdate(
              `${this.apiBaseURL}/update_gaz_detail_inventaire/${this.svData.id}`,
            ...
            this.svData.refUser = this.userData.id;
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_gaz_detail_inventaire`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_gaz_detail_inventaire/${id}`).then(
          ({ data }) => {
      ...
      deleteData(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_gaz_detail_inventaire/${id}`).then(
            ({ data }) => {
              this.showMsg(data.data);
              this.fetchDataList();
            }
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_gaz_details_inventaire/${this.refEnteteVente}?page=`);
      }
