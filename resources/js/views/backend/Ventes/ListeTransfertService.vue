<template>
  <v-layout>
    <!--   -->
    <v-flex md12>
      <VenteDetailTransfert ref="VenteDetailTransfert" />
...
import VenteDetailTransfert from "./VenteDetailTransfert.vue";



export default {
  components:{
    VenteDetailTransfert
  },
  data() {
    return {
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_vente_entete_transfert/${this.svData.id}`,
          ...
          this.svData.author = this.userData.name;
            this.svData.refUser = this.userData.id;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_vente_entete_transfert`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_entete_transfert/${id}`).then(
        ({ data }) => {
          // var donnees = data.data;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_vente_entete_transfert/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    fetchDataList() {
      this.fetch_data(`${this.apiBaseURL}/fetch_vente_entete_transfert_by_service?refUser=`+this.userData.id+"&page="); 
    },
    showDetailTransfert(refEnteteTransfert, name, refService) {

      if (refEnteteTransfert != '') {

        this.$refs.VenteDetailTransfert.$data.etatModal = true;
        this.$refs.VenteDetailTransfert.$data.refEnteteTransfert = refEnteteTransfert;
        this.$refs.VenteDetailTransfert.$data.refService = refService;
        this.$refs.VenteDetailTransfert.$data.svData.refEnteteTransfert = refEnteteTransfert;
        this.$refs.VenteDetailTransfert.fetchDataList();
        this.$refs.VenteDetailTransfert.get_produit_for_service(refService);
        this.$refs.VenteDetailTransfert.fetchListService();
        this.fetchDataList();

        this.$refs.VenteDetailTransfert.$data.titleComponent =
