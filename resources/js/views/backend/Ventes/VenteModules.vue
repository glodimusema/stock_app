            ...
            <v-form ref="form" lazy-validation>
              ...
              <v-card-text>
                <v-text-field label="Designation" prepend-inner-icon="extension" dense
                  :rules="[(v) => !!v || 'Ce champ est requis']" outlined v-model="svData.nom_module">
                </v-text-field>
...
export default {
  ...
  data() {
    return {
      ...
      disabled: false,
      edit: false,
      //'id','nom_module','code_unite','active'
      svData: {
        id: "",
        nom_module: "",
        active: ""
  ...
  methods: {
    ...
    //   }, 300),
    onPageChange() {
      this.fetch_data(`${this.apiBaseURL}/fetch_vente_module?page=`);
    },
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        this.insertOrUpdate(
          `${this.apiBaseURL}/insert_vente_module`,
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_module/${id}`).then(
        ({ data }) => {
    ...
    clearP(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_vente_module/${id}`).then(
          ({ data }) => {
