<template>
  <v-layout>
    <!--   -->
    <v-flex md12>
      <VentePaiementCommande ref="VentePaiementCommande" />

      <v-dialog v-model="dialog" max-width="400px" persistent>
        <v-card :loading="loading">
          <v-form ref="form" lazy-validation>
            <v-card-title>
              Paiement Commande <v-spacer></v-spacer>
      ...
      <v-layout row wrap>
        <v-flex xs12 sm12 md6 lg6>
          <div class="mr-1">
            <router-link :to="'#'">Paiement Commande</router-link>
          </div>
        </v-flex>
      </v-layout>
...
import VentePaiementCommande from "./VentePaiementCommande.vue";

export default {
  components:{
    VentePaiementCommande
  },
  data() {
    return {
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        ...
        if (this.edit) {
          ...
          this.insertOrUpdate(
            `${this.apiBaseURL}/update_vente_entete_paiecommande/${this.svData.id}`,
          ...
          this.svData.refUser = this.userData.id;
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_vente_entete_paiecommande`,
            JSON.stringify(this.svData)
          )
            .then(({ data }) => {
              this.showMsg(data.data);
    ...
    editData(id) {
      this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_entete_paiecommande/${id}`).then(
        ({ data }) => {
          var donnees = data.data;
    ...
    deleteData(id) {
      this.confirmMsg().then(({ res }) => {
        this.delGlobal(`${this.apiBaseURL}/delete_vente_entete_paiecommande/${id}`).then(
          ({ data }) => {
            this.showMsg(data.data);
            this.fetchDataList();
          }
    ...
    showDetailPaiement(refEntetepaie, name, date_paie) {
      ...
      if (refEntetepaie != '') {
        ...
        this.$refs.VentePaiementCommande.$data.titleComponent =
