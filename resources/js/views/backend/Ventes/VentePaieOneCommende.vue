  ...
  export default {
    ...
    data() {
      return {
        ...
        totalPaie: 0,
        RestePaie: 0,
        // 'id','refEntetepaie','refCommande','refBanque','montant_paie','devise',
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.svData.libellepaie = 'Paiement Commande';
            this.insertOrUpdate(
            ...
            this.svData.libellepaie = 'Paiement Commande';
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_vente_paieonecommande`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      // s'id','refCommande','refFrais','puEntree','qteEntree','author'
      //   this.fetchDataList();
      // }, 300),
  
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_paiecommande/${id}`).then(
          ({ data }) => {
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_vente_paieOnecommandeEntete/${this.refCommande}?page=`);
