...
class tvente_entete_paieventeController extends Controller
{
    ...
    { 
        ...
        $data = DB::table('tvente_entete_paievente')
        ->join('tvente_services','tvente_services.id','=','tvente_entete_paievente.refService')
        ->join('tvente_module','tvente_module.id','=','tvente_entete_paievente.module_id')        
        ->select('tvente_entete_paievente.id','code','date_entete_paie','tvente_entete_paievente.author'
        ,'refService','module_id','nom_module','nom_service',
        'tvente_entete_paievente.created_at','tvente_entete_paievente.refUser')
        ->selectRaw('CONCAT("R",YEAR(tvente_entete_paievente.created_at),"",MONTH(tvente_entete_paievente.created_at),"00",tvente_entete_paievente.id) as codePaieCmd');
        if (!is_null($request->get('query'))) {
            # code...
            $query = $this->Gquery($request);

            $data->where('author', 'like', '%'.$query.'%')          
            ->orderBy("tvente_entete_paievente.created_at", "asc");

            return $this->apiData($data->paginate(10));
           

        }
        $data->orderBy("tvente_entete_paievente.created_at", "desc");
        return $this->apiData($data->paginate(10));
    ...
    { 
        ...
        if (!is_null($request->get('query'))) {
            ...
            $data ->where('author', 'like', '%'.$query.'%')          
            ->orderBy("tvente_entete_paievente.created_at", "desc");
            return $this->apiData($data->paginate(10));         
    ...
    function fetch_single_data($id)
    {
        ...
        'tvente_entete_paievente.created_at','tvente_entete_paievente.refUser')
        ->selectRaw('CONCAT("R",YEAR(tvente_entete_paievente.created_at),"",MONTH(tvente_entete_paievente.created_at),"00",tvente_entete_paievente.id) as codePaieCmd')
        ->where('tvente_entete_paievente.id', $id)
        ->get();
    ...
    function update_data(Request $request, $id)
    {
       ...
       {
        $data = tvente_entete_paievente::where('id', $id)->update([           
            'date_entete_paie'    =>  $request->date_entete_paie,
            'refService'    =>  $request->refService,
