  ...
  export default {
    data() {
      return {
        ...
        // 'id','refEntetepaie','refEnteteVente','refBanque','montant_paie','devise',
    ...
    methods: {
      ...
      validate() {
        if (this.$refs.form.validate()) {
          ...
          if (this.edit) {
            ...
            this.svData.libellepaie = 'Paiement Facture Vente';
            this.insertOrUpdate(
              `${this.apiBaseURL}/insert_vente_paiement2`,
              JSON.stringify(this.svData)
            )
              .then(({ data }) => {
                this.showMsg(data.data);
      ...
      fetchDataList() {
        this.fetch_data(`${this.apiBaseURL}/fetch_vente_paiement_byentetepaie/${this.refEntetepaie}?page=`);
      },
      backPage() {
        this.$router.go(-1);
      },
